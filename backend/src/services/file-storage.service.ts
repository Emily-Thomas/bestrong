...
function getLocalStorageDir(): string {
  const storageDir =
    process.env.LOCAL_FILE_STORAGE_DIR ||
    path.join(process.cwd(), 'uploads', 'inbody-scans');
  return storageDir;
}
...
export async function deleteFile(filePathOrUrl: string): Promise<void> {
  if (isLocalDevelopment()) {
    ...
    console.log(
      `File deletion requested for: ${filePathOrUrl} (Vercel Blob - manual deletion may be required)`
    );
  }
}
