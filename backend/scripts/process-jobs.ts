...
if (loaded) {
  ...
  possiblePaths.forEach((p) => {
    ...
    console.log(
      `     - ${abs} ${existsSync(abs) ? '(exists)' : '(not found)'}`
    );
  });
...
if (!process.env.OPENAI_API_KEY) {
  ...
  if (loaded) {
    ...
    console.error(
      '   Please check that OPENAI_API_KEY is defined in your .env file.'
    );
  } else {
...
console.log(
  `✅ OPENAI_API_KEY verified and locked (${apiKeyValue.substring(0, 10)}...)`
);
...
const _onceMode =
  args.includes('--once') || (!watchMode && !args.includes('--watch'));
  ...
  try {
    ...
    for (const job of pendingJobs) {
      try {
        ...
        console.error(
          `❌ Error processing recommendation job ${job.id}:`,
          errorMessage
        );
    ...
    const pendingWeekJobs =
      await weekGenerationJobService.getPendingWeekGenerationJobs();
    console.log(
    ...
    for (const job of pendingWeekJobs) {
      try {
        ...
        console.error(
          `❌ Error processing week generation job ${job.id}:`,
          errorMessage
        );
        if (errorStack) {
...
async function main(): Promise<void> {
  ...
  console.log(
    `   Mode: ${watchMode ? 'watch (continuous)' : 'once (single run)'}`
  );
